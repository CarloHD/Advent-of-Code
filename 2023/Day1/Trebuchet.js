 ...
 *
 *
 * Ogni riga ha dei numeri,
 * bisogna prendere il primo numero della riga e l'ultimo
 * poi bisogna combinarli in modo che ogni riga abbia solo un numero a due cifre
 * poi bisogna sommare tutti i numeri
 */
...
// dividere l'input per ogni riga e ritorna un array di stringhe piene
/** @type {string[]} */
const inputRowArray = input.split('\n').filter(row => row.length !== 0)

/**
 * @description funzione che riceve un array di stringhe, ne filtra i singoli caratteri solo i numeri, quindi ritorna un array di array di numeri
 * @param {string[]} array
 * @returns {number[][]}
 */
const filterStringArrayToNumbMatrices = array =>
  array.map(string =>
    string
      .split('')
      .filter(char => parseInt(char)) // ritornerà falso se NaN, o numero
      .map(value => +value)
  )

/**
 * @description funzione che riceve una matrice di numeri e per ogni sub array ne prende il primo e l'ultimo (anche lo stesso numero) per ritornare un array di soli numeri a due cifre
 * @param {number[][]} matrices
 * @returns {number[]}
 */
const fromMatricesTo2DigitArray = matrices =>
  matrices.map(subArray =>
    parseInt(subArray[0].toString() + subArray[subArray.length - 1].toString())
  )

/**
 * @description funzione che riceve un array di numeri, per poi sommarli
 * @param {number[]} arrayToSum
 * @returns {number}
 */
...
const onlyNumbersMatrices = filterStringArrayToNumbMatrices(inputRowArray)

const twoDigitArray = fromMatricesTo2DigitArray(onlyNumbersMatrices)

const sum = sumArray(twoDigitArray)
 ...
 *
 *
 * ogni riga contiene anche dei numeri scritti a parola
 */

/**
 *  @description funzione che prende una stringa, ne cerca il numero scritto in parola e ne ritorna il suo corrispettivo valore numerico, per poi ritornare l'intera stringa
 * @param {string} string
 * @returns {string}
 */
const fromNumbWordToNumb = string => {
  const numbersWord = [
    'zero',
    'one',
    'two',
  ...
  numbersWord.forEach((word, indexWord) => {
    string = string.replaceAll(
      word,
      word[0] + indexWord + word[word.length - 1]
    )
  })
...
const parsedInputRowArray = inputRowArray.map(row => fromNumbWordToNumb(row))

const onlyNumbersMatrices2 =
  filterStringArrayToNumbMatrices(parsedInputRowArray)

const twoDigitArray2 = fromMatricesTo2DigitArray(onlyNumbersMatrices2)

const sum2 = sumArray(twoDigitArray2)

// Risultati

console.log(sumArray(twoDigitArray)) // 54927 : risposta corretta!
console.log(sumArray(twoDigitArray2)) // 54581 : risposta corretta!
