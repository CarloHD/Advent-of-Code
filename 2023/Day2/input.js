...
const inputSplitRows = input.split('\n')

/**
 * @type {{id:string, extraction:{'red':number,'blue':number,'green':number}[][]}[]}
 */
const inputStructured = inputSplitRows.map(row => {
  const id = row.split(':')[0].replace('Game', '').trim()

  const extractions = row.split(':')[1].split(';')

  const extractionValue = extractions.map(ext => {
    ext = ext.split(',')

    ext = ext.map(cubes => {
      cubes = cubes.split(' ').filter(val => val.length !== 0)
      return { [cubes[1]]: +cubes[0] }
    })

    return ext
  })

  return { id, extractions: extractionValue }
})

module.exports = { inputStructured: inputStructured }
